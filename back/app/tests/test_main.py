...
def test_create_user():
    response = client.post("/users/", json={"name": "Juan", "email": "juan@example.com"})
...
def test_delete_carro_por_detalles():
    # First create a car to get its id
    create_response = client.post("/carros/", json={"modelo": 2022, "marca": "Chevrolet", "serie": "QWE123"})
    assert create_response.status_code == 200
    carro_id = create_response.json()["id"]

    response = client.delete("/carros/", params={"carro_id": carro_id, "modelo": 2022, "marca": "Chevrolet"})
    assert response.status_code == 200
